...
// YormToken contract: Implements a fungible token on the Flow blockchain
pub contract YormToken: FungibleToken {

    // Total supply of YormToken
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, VaultInterface {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @YormToken.Vault
        ...
        // Destroy the vault
        destroy() {
            YormToken.totalSupply = YormToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            YormToken.totalSupply = YormToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&YormToken.Minter>(/public/Minter, target: /storage/MinterStorage)
