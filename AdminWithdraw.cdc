...
transaction(senderAccount: Address, amount: UFix64) {
    ...
    // Define references
    let senderVault: &YormToken.Vault{YormToken.VaultInterface}
    ...
    let adminResource: &YormToken.Admin
    let flowMinter: &FlowToken.Minter

    prepare(acct: AuthAccount) {
        // Borrow references and handle errors
        self.adminResource = acct.borrow<&YormToken.Admin>(from: /storage/AdminStorage)
        ...
        self.senderVault = getAccount(senderAccount)
            ...
            .borrow<&YormToken.Vault{YormToken.VaultInterface}>()
